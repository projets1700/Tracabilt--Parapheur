...
export default function PageVisionneur() {
  ...
  const evenementsGPS = resultat?.evenements?.filter(e => e.latitude && e.longitude) || [];
  const pointsCarte = evenementsGPS.map(e => [parseFloat(e.latitude), parseFloat(e.longitude)]);
      ...
      <main className="conteneur" style={{ padding: '24px 24px' }}>
        ...
        {!resultat && !chargement && !erreur && (
          <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--texte3)' }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 500 }}>Saisissez un numéro de parapheur</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 800, margin: '0 auto' }}>
            ...
            <div className="carte">
              <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>
                Historique des déplacements ({resultat.evenements?.length || 0})
              </h3>
              {!resultat.evenements?.length ? (
